 ...
 * @since 1.0
 */
public class TaskFlowService implements ITaskService {
    ...
    public Task complete(String taskId, String operator, Map<String, Object> args) {
        ...
        HistTask history = CglibUtil.copy(task, HistTask.class);
        history.setFinishTime(DateHelper.getTime());
        history.setTaskState(FlowState.STATE_FINISH.getState());
        history.setOperator(operator);
        if (history.getActorIds() == null) {
            List<TaskActor> actors = taskActorService.getTaskActorsByTaskId(task.getId());
     ...
     * @return 历史任务对象
     */
    public HistTask history(Execution execution, CustomModel model) {
        HistTask historyTask = new HistTask();
        ...
        historyTask.setTaskState(FlowState.STATE_FINISH.getState());
        ...
        histTaskService.save(historyTask);
        return historyTask;
    }
    ...
    public Task resume(String taskId, String operator) {
        HistTask histTask = histTaskService.getById(taskId);
    ...
    public void addTaskActor(String taskId, Integer performType, String... actors) {
        ...
        switch (performType) {
            case 0:
                ...
                Map<String, Object> data = JsonHelper.fromJson(task.getVariable(), Map.class);
            ...
            case 1:
                try {
                    for (String actor : actors) {
                        Task newTask = CglibUtil.copy(task, Task.class);
                        newTask.setId(StringHelper.getPrimaryKey());
                        newTask.setCreateTime(DateHelper.getTime());
                        newTask.setOperator(actor);
                        Map<String, Object> taskData = JsonHelper.fromJson(task.getVariable(), Map.class);
                        taskData.put(Task.KEY_ACTOR, actor);
                        //todo bug
                        newTask.setVariable(JsonHelper.toJson(taskData));
    ...
    public void removeTaskActor(String taskId, String... actors) {
        ...
        if (task.isMajor()) {
            taskActorService.removeTaskActor(task.getId(), actors);
            Map<String, Object> taskData = JsonHelper.fromJson(task.getVariable(), Map.class);
    ...
    public Task withdrawTask(String taskId, String operator) {
        ...
        if (hist.isPerformAny()) {
            tasks = taskService.getNextActiveTasks(hist.getId());
        } else {
            tasks = taskService.getNextActiveTasks(hist.getOrderId(),
    ...
    public Task rejectTask(ProcessModel model, Task currentTask) {
        ...
        //
        HistTask history = histTaskService.getById(parentTaskId);
    ...
    private void assignTask(String taskId, String... actorIds) {
        ...
        for (String actorId : actorIds) {
            ...
            taskActorService.save(taskActor);
        }
    }
    ...
    public TaskModel getTaskModel(String taskId) {
        ...
        Process process = processService.getById(order.getProcessId());
    ...
    public List<Task> createTask(TaskModel taskModel, Execution execution) {
        ...
        if (taskModel.isPerformAny()) {
            ...
            for (String actor : actors) {
                Task singleTask;
                try {
                    singleTask = CglibUtil.copy(task, Task.class);
                } catch (Exception e) {
                    singleTask = task;
                }
